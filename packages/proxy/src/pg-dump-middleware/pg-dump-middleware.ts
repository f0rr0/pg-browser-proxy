...
import {
  isGetExtensionsQuery,
  patchGetExtensionsResult,
} from "./get-extensions-query.ts";
...
class PgDumpMiddleware {
  ...
  server(
    ...
    if (
      context.clientParams?.application_name !== "pg_dump" ||
      !this.state.has(connectionId)
    ) {
      return message;
    }
    ...
    switch (connectionState?.step) {
      case "get-extensions-query-received": {
        ...
        if (patched.vectorOid) {
          if (Number.parseInt(patched.vectorOid) >= FIRST_NORMAL_OID) {
            this.state.set(connectionId, {
              step: "complete",
            });
          } else {
      ...
      case "get-extension-membership-query-received": {
        const patchedMessage = patchGetExtensionMembershipResult(
          message,
          connectionState.vectorOid,
        );
